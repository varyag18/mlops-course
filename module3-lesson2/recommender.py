...
class RecommenderModel:
    ...
    def fit(self, df: pd.DataFrame, df_year: pd.DataFrame):
        ...
        df_distinct = df.drop_duplicates('track_id').sort_values('track_id').reset_index(drop=True)
        data_encoded = pd.concat([df_distinct, xtab_song.drop(columns=['track_id'])], axis=1)
        ...
        self.similarity_matrix = cosine_similarity(data_encoded[numerical_features + list(xtab_song.columns[1:])])

    def recommend(self, track_title: str, N: int = 5):
        indices = pd.Series(self.data_encoded.index, index=self.data_encoded['track_name']).drop_duplicates()
        ...
        recommended = self.data_encoded[['track_name', 'artists', 'album_name']].iloc[song_indices]
